...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!isSubscribed && existingBoard.length >= ORG_BOARD_LIMIT) {
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (title.length > 60) {
      throw new Error("Title cannot be longer then 60 character");
    }
    // TODO: later chekc to delete fav. relation as well
